...
class Record:
    ...
    def __init__(self, gpu_id, sample_interval, out_file_name, sup_gpu_num=-1):
        print("start a record object**********************")
        self.gpu_id = gpu_id
        self.sample_interval = sample_interval
        self.out_file_name = out_file_name
        self.sup_gpu_num=sup_gpu_num
    ...
    def run(self):
        ...
        while True:
            ...
            if self.gpu_id == -1:
                device_count=self.sup_gpu_num if self.sup_gpu_num != -1 else torch.cuda.device_count()
                # print("GPU num:",device_count)
...
parser.add_argument("--spe_gpu_num",default=-1,type=int)
args=parser.parse_args()
spe_gpu_num=args.spe_gpu_num
...
if torch.cuda.is_available():
    ...
    device_name="CPU"
model_name="datacollectonly"
sample_interval=1

out_file_name=model_name+"-"+device_name+"-"+sample_interval.__str__()+"-"+formatted_time+".txt"
print(out_file_name)


record=Record(-1,sample_interval,out_file_name,spe_gpu_num)
record.run()