...
class _Packing(object):
    ...
    def calc_iteration_time(self, packing=None, ordering=1):
        ...
        if ordering==1: # best ordering
            ...
            for permutation in jobs_permutation:
                TT = 0.0
                if multi_resource==4 and len(jobs[0].resource_time)==3:
                    round = 2
                    ...
                    for i in range(multi_resource):
                        max_num = 0.0
                        for idx, val in enumerate(permutation):
                            
                            max_num = max(max_num, val.resource_time[(i-idx+multi_resource)%multi_resource])
                        TT += max_num
                if TT < TT_all:
                    TT_all = TT
                    self.best_permutation = copy.deepcopy(permutation)
            if len(jobs)==2 and multi_resource==4 and len(jobs[0].resource_time)==3:
    ...
    def calc_used_ratio(self, packing, ordering=1):
        ...
        return used_time_sum/(TT*multi_resource)
