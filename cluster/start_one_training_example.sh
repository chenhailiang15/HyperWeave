...
net_card="eno1"           # Please modified to your Network Card name
MASTER_ADDR="localhost"   # Please modified according to your need (one node:localhost  multi node: master ip)
MASTER_PORT="12345"       # Please modified according to your need (communication port)
...
gpu_id_list="[[4],[]]"    # List of GPU IDs for each node participating in the training.
...
python platform/HyperWeaveExecutor.py --MASTER_ADDR ${MASTER_ADDR} --MASTER_PORT ${MASTER_PORT} --net_card ${net_card}  --model_name ${model_name} --node_rank ${node_rank} \
